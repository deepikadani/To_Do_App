...
class App extends Component {
  ...
  addStudent = () => {
    ...
    let mobile = this.mobileInput.value.trim();
    let branch = this.branchInput.value.trim();

    if (!roll || !name || !mobile || branch === "Select branch") {
      alert("Please fill all details!");
      return;
    }


    if (isNaN(roll) || isNaN(mobile)) {
      alert("Roll number and Contact number must be valid numbers!");
      return;
    }


    const isRollExists = this.state.studentList.some(
    ...
    const isMobileExists = this.state.studentList.some(
    ...
    this.branchInput.value = "Select branch";
  }
